...
-   To display the status of an ID when calling up a navigation or a submenu, you can enter the following entry in the
    ...
    ID - must be replaced with the ID to be queried.**_Breaking Change!!!_**The parameter true behind it says or the value can be changed by`change{"true":"an","false":"aus"}`or a modified version thereof. This is important if I have more than one
    If I want to query the status in one call, I don't set the value and the values ​​are not processed properly.<br>
...
-   If you send several diagrams, the file name must be different, otherwise the images will be mutually exclusive
    overwrite
-   Delay the time between the request and the sending of the image -> depending on the speed of the system can and must be a
    other value can be taken
...
-   Integrated event listener: Waits for a data point - this data point is set (e.g. via script or
    Adapter), a predefined menu is opened. It is checked for the condition and ack which one is used for each
    Event can be entered separately.
...
-   Resize Keyboard - Prompts customers to resize the keyboard vertically for optimal fit
