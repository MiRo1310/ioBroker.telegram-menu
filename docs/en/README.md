...
![Navigation](../pic/nav. ng)<br>Here you can see the beginning of the navigation, line 1 (green) is the start navigation, this is sent when the adapter is started or restarted.
...
![Buttons in Telegram](../pic/image-1. ng)<br>Here is the sent menu in Telegram. For example, if I now press heating, "Heating" is sent as a text to the adapter, which looks for the appropriate call text, which must be written exactly like that, see the picture above.**Very important, each name of the call text may only appear once, i. . it must be unique**

![Benutzername](../pic/image. ng)
...
-   Various predefined submenus can be used, e. . on-off, percent or numbers for e. . the roller shutter control. A new trigger is automatically created in the actions, but more on that below.

-   You can jump from one menu to another menu. Makes sense if two people have Menu 1 together, but User1 should get another menu to which User2 should not have access. This corresponding button would be visible for both, only for User1 with a function. To do this, the corresponding user must be specified in both groups.![Menu1](../pic/image7. ng)<br>This is the first menu, here the menu is sent when the adapter starts<br>![Menu2](../pic/image8. ng)<br>This is the second menu, for this to work the call text on the homepage must be deactivated. This can be achieved by simply entering`-`enters.
    Now user Michael can access Menu2 from Menu1 by pressing the Light button. You can return to Menu1 using the new Home button that appears.**Important!! Even if there are two menus, each call text can only appear once!**If there are two menus that do not have the same user, each menu can of course have an entry, e. . light, but not if you jump from one to the other.

#### delete history

To delete all messages (similar to "Delete history" in the client), add a menu item`menu:deleteAll:Navigation`-**Navigation**is the menu name that should then be called up (e. . home page)

#### Status

To display the status of an ID when calling up a navigation or a submenu, the following entry in the text field can be used.<br>![Config for Status](../pic/statusConfig. ng)The result would then be this!<br>![Telegram Status](../pic/TelegramStatus. ng)

#### Icons in den Menu-Buttons

![Icon1](../pic/heizung-icon1. ng)

If you want to have special icons in the menu buttons, copy an emoji (e. .<https://www. etemojis. et/html/#Emoji-Liste>) and uses it like a character. The code of the emoji is not copied, but the emoji directly!

![Icon2](../pic/heizung-icon2. ng)

### Sub-menus

![Submenus](../pic/image10. ng)
...
-   The name must always be a unique name, so it can only appear once and then refers to the trigger in action, where the ID is specified.
     ...
     menu:switch-on. rue-off. alse:name:
...
-   The 1. 0 indicates the range, the 2 the steps, and the unit the unit, everything can be replaced variably. e. .`menu:number16-36-4-°C:temperaturXY:`
...
-   Goes back to the previous page, you can go back a maximum of 20 pages
    ...
    <br>
    <img src="../pic/menu_percent10_r2. ng" width="800"/>
    <img src="../pic/submenu_setstate. ng" width="800"/>

### SetState

![SetState](../pic/image3. ng)
...
-   If I want to read a value from a data point, but have to convert the value, I can put it in the return text`{math:/10}`for example, here we divide by 10![math](../pic/image9. ng)<br>
...
-   If you want to have the name of the data point in the output text, simply enter it at the desired position in the text`{common. ame}`a

![functions](../pic/functions. ng)<br>
...
![InlineTable](../pic/inlinetable-grafik. ng)![TextTable](../pic/textable-grafik. ng)

### Send Picture

-   You can insert a token for Grafana in the settings
-   A directory must be created in which you have all rights, e. .`/opt/iobroker/grafana/`to be able to temporarily save the images there
...
-   Delay is the time between the request and the sending of the image -> depending on the speed of the system, a different value can and must be used
    ...
    <img src="../pic/grafana. ng" width="400">

### Events

-   Integrated event listener: Waits for a data point - if this data point is set (e. . via script or adapter), a predefined menu is opened. The condition and Ack are checked, which can be entered separately for each event.
...
-   Theme, different themes can be set from the Echarts adapter, e. .`auto, default, dark, dark-bold, dark-blue, gray, vintage, macarons, infographic, shine, roma, azul, blue, royal, tech-blue, red, red-velvet, green`
