...
async function saveMessageIds(state, instanceTelegram) {
    try {
        ...
        if (requestMessageIdObj?.val) {
            const result = (0, string_1.parseJSON)(String(requestMessageIdObj?.val), main_1.adapter);
            json = result.json;
...
async function deleteMessageIds(instance, user, telegramParams, whatShouldDelete) {
    ...
    try {
        ...
        const lastMessageId = await main_1.adapter.getForeignStateAsync(`${instance}.communicate.requestMessageId`);
        ...
        json[chat_id].forEach((element, index) => {
            ...
            if (whatShouldDelete === 'all' && id) {
                (0, botAction_1.deleteMessageByBot)(instance, user, parseInt(id), chat_id);
            }
