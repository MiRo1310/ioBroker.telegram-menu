 ...
 */
async function checkEveryMenuForData(_this, menuData, calledValue, userToSend, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, menus, userActiveCheckbox, token, directoryPicture, timeoutKey) {
    // Alle Menus durchgehen und nach Daten suchen
    for (const menu of menus) {
        let groupData = {};
        //@ts-ignore
        if (menuData.data) {
            groupData = menuData.data[menu];
        }
        //@ts-ignore
        else if (menuData[menu]) {
            groupData = menuData[menu];
        }
        ;
        ...
        if (await processData(_this, groupData, calledValue, userToSend, menu, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, 
        //@ts-ignore
        menuData.data, menus, menu, userActiveCheckbox, token, directoryPicture, timeoutKey)) {
 ...
 */
async function processData(_this, menuData, calledValue, userToSend = "", groupWithUser, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, allMenusWithData, menus, menu, userActiveCheckbox, token, directoryPicture, timeoutKey) {
    try {
        let part = {};
        let call;
        ...
        if (getDynamicValue(userToSend)) {
            ...
            let valueToSet;
            if (res.valueType)
                valueToSet = adjustValueType(_this, calledValue, res.valueType);
            else
                valueToSet = calledValue;
            if (valueToSet)
                await _this.setForeignStateAsync(res.id, valueToSet, res.ack);
            else
                sendToTelegram(_this, userToSend, `You insert a wrong Type of value, please insert type: ${res.valueType}`, undefined, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, "");
        ...
        if (menuData[call] && !calledValue.includes("menu:") && userToSend && groupWithUser && userActiveCheckbox[groupWithUser]) {
            ...
            if (part.nav) {
                ...
                if (JSON.stringify(part.nav).includes("menu:")) {
                    ...
                    const result = await callSubMenu(_this, JSON.stringify(part.nav), menuData, userToSend, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, part, allMenusWithData, menus, setStateIdsToListenTo);
                    if (result && result.setStateIdsToListenTo)
                        setStateIdsToListenTo = result.setStateIdsToListenTo;
                    if (result && result.newNav) {
                        checkEveryMenuForData(_this, allMenusWithData, result.newNav, userToSend, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, menus, userActiveCheckbox, token, directoryPicture, timeoutKey);
                    }
                }
                else {
                    sendNav(_this, part, userToSend, instanceTelegram, userListWithChatID, resize_keyboard, one_time_keyboard);
            ...
            // Schalten
                ...
                const result = sendPic(_this, part, userToSend, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, token, directoryPicture, timeouts, timeoutKey);
                if (result)
                    timeouts = result;
                else
            ...
            const result = await callSubMenu(_this, calledValue, menuData, userToSend, instanceTelegram, resize_keyboard, one_time_keyboard, userListWithChatID, part, allMenusWithData, menus, setStateIdsToListenTo);
            if (result && result.setStateIdsToListenTo)
                setStateIdsToListenTo = result.setStateIdsToListenTo;
            return true;
        }
