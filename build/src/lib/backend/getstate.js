            ...
            if (id.indexOf(specificatedSelektor) != -1) {
                ...
                _this.getForeignStateAsync(id).then(async (value) => {
                    if (value) {
                        ...
                        if (element.text) {
                            ...
                            if (textToSend.includes("{json")) {
                                if (decomposeText(textToSend, "{json", "}").substring.includes("TextTable")) {
                                    ...
                                    if (valueForJson && valueForJson.length > 0) {
                                        ...
                                        sendToTelegram(_this, userToSend, "The state is empty!", undefined, telegramInstance, one_time_keyboard, resize_keyboard, userListWithChatID, parse_mode);
                            ...
                            if (textToSend.indexOf("&&") != -1)
                                text += `${textToSend.replace("&&", val)}${newline}`;
                            else
                    ...
                    if (i == part.getData?.length) {
                        ...
                        if (userToSend)
                            sendToTelegram(_this, userToSend, text, undefined, telegramInstance, one_time_keyboard, resize_keyboard, userListWithChatID, parse_mode);
                    }
                    i++;
                });
            }
