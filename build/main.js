...
class TelegramMenu extends utils.Adapter {
    ...
    async onReady() {
        ...
        const { directoryPicture, telegramParams, menusWithUsers, listOfMenus, isUserActiveCheckbox, checkboxNoEntryFound, textNoEntryFound, sendMenuAfterRestart, token, dataObject, checkboxes, } = (0, configVariables_1.getConfigVariables)(this.config, exports.adapter);
        ...
        try {
            if (!(await (0, connection_1.areAllCheckTelegramInstancesActive)(telegramParams))) {
            ...
            for (const name in nav) {
                ...
                if (generatedActions) {
                    ...
                    if (subscribeForeignStateIds?.length) {
                        await (0, subscribeStates_1._subscribeForeignStates)(exports.adapter, subscribeForeignStateIds);
                    }
                }
                else {
                ...
                if (events) {
                    for (const event of events) {
                        await (0, subscribeStates_1._subscribeForeignStates)(exports.adapter, event.ID);
                    }
                }
            ...
            if (sendMenuAfterRestart) {
                await (0, adapterStartMenuSend_1.adapterStartMenuSend)(listOfMenus, startSides, isUserActiveCheckbox, menusWithUsers, menuData, telegramParams);
            }
            let menus = [];
            this.on('stateChange', async (id, state) => {
                ...
                const { isEvent, eventUserList } = (0, events_1.getInstancesFromEventsById)(dataObject.action, id, menusWithUsers);
                if (isEvent && state) {
                    for (const user of eventUserList) {
                        await (0, action_1.handleEvent)(exports.adapter, user, dataObject, id, state, menuData, telegramParams);
                    }
                }
                if (!instance) {
                    return;
                ...
                if (this.isMessageID(id, telegramBotSendMessageID(instance), telegramRequestMessageID(instance))) {
                    await (0, messageIds_1.saveMessageIds)(exports.adapter, state, instance);
                }
    ...
    async checkInfoConnection(id, telegramParams) {
        try {
            ...
            (0, logging_1.errorLogger)('Error checkInfoConnection', e, exports.adapter);
            return null;
        }
    }
    ...
    onUnload(callback) {
        ...
        try {
            ...
            (0, logging_1.errorLogger)(e, 'Error onUnload', exports.adapter);
            callback();
        }
    }
    onMessage(obj) {
