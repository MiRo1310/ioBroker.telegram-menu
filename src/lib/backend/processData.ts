...
let setStateIdsToListenTo: SetStateIdsToListenTo[] = [];
...
async function checkEveryMenuForData(
	...
	for (const menu of menus) {
		let groupData: DataObject | { nav: string[] } = {} as DataObject | { nav: string[] };
		//@ts-ignore
...
async function processData(
	_this: any,
	menuData: DataObject | { nav: string[] },
	calledValue: string,
	userToSend = "",
	...
	try {
		...
		//@ts-ignore
		if (menuData[call] && !calledValue.includes("menu:") && userToSend && groupWithUser && userActiveCheckbox[groupWithUser as keyof IsUserActiveCheckbox]) {
			//@ts-ignore
			part = menuData[call];
			// Navigation
			if (part.nav) {
