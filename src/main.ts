...
import { generateActions, getUserToSendFromUserListWithChatID } from './app/action';
...
import { getInstancesFromEventsById, handleEvent } from './app/events';
import { findDeprecatedAndLog } from './app/deprecated';
    ...
    private async onReady(): Promise<void> {
        ...
        try {
            ...
            this.on('stateChange', async (id, state) => {
                ...
                const { isEvent, eventUserList } = getInstancesFromEventsById(dataObject.action, id, menusWithUsers);
                if (isEvent && state) {
