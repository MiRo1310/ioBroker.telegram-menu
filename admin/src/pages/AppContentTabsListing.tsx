...
class TabListing extends Component<PropsMainTabList> {
    ...
    getTabs = () => {
        tabValues.map((tab, index) => {
            if (this.tabs.find(t => t.value === tab.value)) return
    ...
    isActive = (val: string): boolean => {
        return this.props.data.state.tab === val || (this.props.data.state.subTab === val && this.props.data.state.tab === "action");
    }
                ...
                <div className={"flex justify-between App-TabList"}>
                    <div className={"flex flex-wrap"}>
                        {this.getTabs().map(tab => (
                            <button key={tab.label} className={`${this.isActive(tab.value) ? 'active' : ''}`}
                                    onClick={() => this.handleChange(tab.value)}>{I18n.t(tab.label)}</button>
                        ))}
                    </div>
                    <button key={"settings"} className={`icon ${this.isActive("settings") ? 'active' : ''}`}
                            onClick={() => this.handleChange("settings")}><i
