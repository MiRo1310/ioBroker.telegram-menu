				...
				_this.getForeignStateAsync(element.id).then((value) => {
					...
					if (value) {
						...
						if (element.text) {
							...
							console.log(val);
							let resultChange = changeValue(textToSend, val, _this);
							...
							console.log("TextToSend " + JSON.stringify(textToSend));
					...
					if (i == part.getData.length) {
						...
						if (userToSend) sendToTelegram(_this, userToSend, text, undefined, telegramInstance, one_time_keyboard, resize_keyboard, userListWithChatID);
					}
					i++;
				});
			}
