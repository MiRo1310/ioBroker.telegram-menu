 ...
 */
function sendToTelegram(_this, user = "", value, keyboard = [], instance = "telegram.0", resize_keyboard = true, one_time_keyboard = true, userListWithChatID, parse_mode) {
	try {
		...
		if (parse_mode === "true" || parse_mode === true) parse_mode = "HTML";
		else parse_mode = "";
 ...
 */
function sendToTelegramSubmenu(_this, user, text, keyboard, instance = "telegram.0", userListWithChatID, parse_mode) {
	try {
		let chatId = "";
		userListWithChatID.forEach((element) => {
			if (element.name === user) chatId = element.chatID;
