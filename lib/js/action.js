			...
			menuArray.forEach((menu) => {
				if (usersInGroup[menu] && menuData.data[menu][calledNav]) {
					usersInGroup[menu].forEach((user) => {
						...
						_this.log.debug("test " + JSON.stringify(part.nav[0]));
						_this.log.debug("test " + JSON.stringify(typeof part.nav[0]));
						_this.log.debug("test " + JSON.stringify(part.nav[0].includes("menu:")));
