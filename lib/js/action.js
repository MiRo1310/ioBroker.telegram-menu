 ...
 * @param {string} user
 */
const lichtAn = async (_this, user) => {
	let light_list = Array.prototype.slice.apply($("state[id=*](functions=licht)"));
	let listLightsOn = "";

	for (const element of light_list) {
		let obj = await _this.getObjectAsync(element);
		if (obj) {
			let val = await _this.getStateAsync(obj?._id);
			if (val && val.val) {
				let text = ` ${obj.common.name}: ${val.val} \n`
					.replace("_", " ")
					.replace("_", " ")
					.replace("POWER", "");
				listLightsOn += text;
...
const wertUebermitteln = async (_this, user, id, textVorVal, textNachVal) => {
	let value = await _this.getStateAsync(id);
...
function generateActions(_this, action, nameObj) {
	...
	action.set.forEach(function (element, key) {
		...
		element.IDs.forEach(function (id, key) {
			let toggle = element.checkboxes[key] === "true";
			let value;
