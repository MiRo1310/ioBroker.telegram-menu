...
const { sendToTelegramSubmenu } = require("./telegram.js");
const { subMenu } = require("./subMenu.js");
const { sendNav } = require("./senNav.js");
...
const checkEvent = (dataObject, id, state, menuData, _this, userListWithChatID, instanceTelegram, resize_keyboard, one_time_keyboard, usersInGroup, textNoEntryFound) => {
	const menuArray = [];
	let ok = false;
	let calledNav = "";
			...
			menuArray.forEach((menu) => {
				if (usersInGroup[menu] && menuData.data[menu][calledNav]) {
					usersInGroup[menu].forEach((user) => {
						...
						const value = JSON.stringify([part.nav[0]]);
