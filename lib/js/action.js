...
function exchangeValue(textToSend, stateVal, _this) {
	const startindex = Utils.decomposeText(textToSend, "change{", "}").startindex;
	const endindex = Utils.decomposeText(textToSend, "change{", "}").endindex;
...
const createKeyboardFromJson = (_this, val, text) => {
	try {
		const array = Utils.decomposeText(text, "{json:", "}").substring.split(";");
...
function createTextTableFromJson(_this, val, textToSend) {
	try {
		if (!val) return;
		const substring = Utils.decomposeText(textToSend, "{json:", "}").substring;
		const array = substring.split(";");
		const itemArray = array[1].replace("[", "").replace("]", "").replace(/"/g, "").split(",");
		const valArray = JSON.parse(val);
		const lengthArray = [];
...
const setDynamicValue = (_this, returnText, ack, id, userToSend, telegramInstance, one_time_keyboard, resize_keyboard, userListWithChatID, parse_mode, confirm) => {
	const substring = Utils.decomposeText(returnText, "{setDynamicValue:", "}").substring;
	const array = substring.split(":");
	const text = array[1];
