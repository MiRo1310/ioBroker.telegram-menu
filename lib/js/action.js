...
async function editArrayButtons(val, _this) {
	try {
		...
		_this.log.error("Error EditArray: " + err.message);
	}
}
				...
				if (enums) {
					const promises = enums.map(async (id) => {
						try {
							...
							_this.log.error("Error idBySelector: " + JSON.stringify(e.message));
						}
					});
					Promise.all(promises)
						...
						.catch((e) => {
							_this.log.debug("Error " + JSON.stringify(e.message));
						});
				}
			}
		}
...
async function generateNewObjectStructure(_this, val) {
	try {
		...
		_this.log.error("Error GenerateNewObjectStructure " + err.message);
	}
}
...
function generateActions(_this, action, userObject) {
	try {
		...
		_this.log.debug("Error generateActions" + JSON.stringify(err.message));
	}
}
