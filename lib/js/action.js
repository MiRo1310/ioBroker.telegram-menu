...
const processTimeValue = (value, textToSend) => {
...
const processTimeIdLc = async (textToSend, id, _this) => {
	...
	let timeValue;
	let timeStringUser;
	if (textToSend.includes("{time.lc")) {
		timeStringUser = textToSend.replace("{time.lc,(", "").replace(")}", "");
		timeValue = value.lc;
	} else if (textToSend.includes("{time.ts")) {
		timeStringUser = textToSend.replace("{time.ts,(", "").replace(")}", "");
		timeValue = value.ts;
	}
	...
	const time = {
		ms: milliceconds < 10 ? "000" + milliceconds : milliceconds < 100 ? "00" + milliceconds : milliceconds < 1000 ? "0" + milliceconds : milliceconds,
