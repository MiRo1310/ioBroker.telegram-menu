				...
				action[item.objName].forEach(function (element, key) {
					...
					element[item.loop].forEach(function (id, key) {
						...
						if (item.objName == "set") {
							...
							toggle = element.switch_checkbox[key] === "true";

							// Aus true oder false einen boolean machen
							if (element.values[key] === "true" || element.values[key] === "false") {
								valueSet = element.values[key] === "true";
							} else valueSet = element.values[key];
						}
						...
						item.elements.forEach((elementItem) => {
							...
							if (elementItem.name == "toggle") val = toggle;
							else if (item.objName == "set") val = valueSet;
							else if (!element[value]) val = false;
