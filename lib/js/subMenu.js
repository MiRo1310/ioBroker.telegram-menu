...
async function callSubMenu(
	...
	try {
		...
		if (subMenuData && typeof subMenuData[0] == "string" && subMenuData[1] && typeof subMenuData[1] == "object") {
 ...
 * @param {array} menus - Array with all menus
	...
	try {
		...
		if (callbackData.includes("delete")) {
			...
			if (callbackData.includes("deleteAll")) {
				deleteMessageIds(_this, userToSend, userListWithChatID, instanceTelegram, "all", null);
			...
			else _this.log.error("Please insert a Menu in your Delete Submenu");
			return;
