 ...
 * @param {number|null} leaveLastStanding Leave last message standing
 */
async function deleteMessageIds(_this, user, userListWithChatID, instanceTelegram, whatShouldDelete, leaveLastStanding) {
	const requestMessageIdObj = await _this.getStateAsync("communication.requestIds");
	...
	if (requestMessageIdObj && JSON.parse(requestMessageIdObj.val)) {
		...
		messageIds[chat_id].forEach((element, index) => {
			if (whatShouldDelete === "all") deleteMessageByBot(_this, instanceTelegram, user, userListWithChatID, element.id, chat_id);
			else if (whatShouldDelete === "last" && index === 0) deleteMessageByBot(_this, instanceTelegram, user, userListWithChatID, element.id, chat_id);
			else if (whatShouldDelete === "leaveL" && leaveLastStanding && index <= leaveLastStanding - 1)
